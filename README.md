...
| 结果图标注 | Top 3 绿色轮廓 + `序号 · 亮度值`，其余细胞黄色轮廓 + 白色编号 |
| 环形采样区域可视化 | 直观展示亮度计算范围 |
| 原图 / 结果图切换 | 一键对比原始图像与分析结果 |
| 数据保存 | 一键保存结果图 + Top3 坐标 + 所有细胞直径数据 |
...
4. 点击 **细胞识别** 开始分割
5. 查看结果：Top 3 细胞高亮标注，状态栏显示检测/有效细胞数
6. 点击 **保存** 导出数据到 `细胞数据/` 目录
...
└── 原文件名/
    ...
    ├── 原文件名_top3.txt      ← 亮度前三坐标与亮度值
    └── 原文件名_data.txt      ← 所有有效细胞编号、直径、坐标
```
